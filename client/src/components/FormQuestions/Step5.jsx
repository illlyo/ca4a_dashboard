...
class Step5 extends React.Component {
  ...
  if (Object.keys(validateNewInput).every((k) => { return validateNewInput[k] === true })) {
      if (this.props.getStore().learning_trajectory_success_challenge != userInput.learning_trajectory_success_challenge ||
          this.props.getStore().in_between_steps != userInput.in_between_steps ||
          this.props.getStore().inquiry_institute != userInput.inquiry_institute ||
          this.props.getStore().research != userInput.research ||
          this.props.getStore().design != userInput.design ||
          this.props.getStore().pdsa != userInput.pdsa ||
          this.props.getStore().synthesize != userInput.synthesize ||
          this.props.getStore().scale != userInput.scale ) { // only update store of something changed
  ...
  _validationErrors(val) {
    const errMsgs = {
      learning_trajectory_success_challengeValMsg: val.learning_trajectory_success_challengeVal ? '' : 'Response required',
      in_between_stepsValMsg: val.in_between_stepsVal ? '' : 'Response required',
      inquiry_instituteValMsg: val.inquiry_instituteVal ? '' : 'Response required',
      researchValMsg: val.researchVal ? '' : 'Response required',
      designValMsg: val.designVal ? '' : 'Response required',
      pdsaValMsg: val.pdsaVal ? '' : 'Response required',
      synthesizeValMsg: val.synthesizeVal ? '' : 'Response required',
      scaleValMsg: val.scaleVal ? '' : 'Response required'
    }
    console.log(errMsgs)
    return errMsgs;
  }
            ...
            <div className="row content">
              <div className="col-md-12">
                <div className="form-style-10">
                  ...
                  <div className="section"><span>16</span>What successes/challenges are you experiencing in moving this team through their Learning Trajectory?</div>
                    <div className="inner-wrap">
                      <div className={notValidClasses.learning_trajectory_success_challengeCls}>
                        <textarea type="string"
                                  name="field3"
                                  onBlur={this.validationCheck}
                                  ref="learning_trajectory_success_challenge"
                                  defaultValue={this.state.learning_trajectory_success_challenge} />
                    ...
                    <div className="section"><span>17</span>What are the "in-between" steps you can take before your next meeting to move this team's work forward?</div>
                    <div className="inner-wrap">
                      <div className={notValidClasses.in_between_stepsCls}>
                        <textarea type="string"
                                  name="field3"
                                  onBlur={this.validationCheck}
                                  ref="in_between_steps"
                                  defaultValue={this.state.in_between_steps} />
                          <div className={notValidClasses.in_between_stepsValGrpCls}>{this.state.in_between_stepsValMsg}</div>
                      </div>
                    </div>

                    <div className="section"><span>18</span>Where do you see the team going in their Learning Trajectory in your next meeting?</div>
                    <div className="inner-wrap">
                            <label>Check all that apply*</label>
                              <input type="checkbox"
                                     className="form-control"
                                     ref="inquiry_institute"
                                     defaultValue={this.state.inquiry_institute}
                                     onBlur={this.validationCheck}
                                     onChange={this.onChangeOne} />Inquiry Institute - (Develop a learning trajectory and change idea)<br></br>
                              <input type="checkbox"
                                     className="form-control"
                                     ref="research"
                                     defaultValue={this.state.research}
                                     onBlur={this.validationCheck}
                                     onChange={this.onChangeTwo} />Research - (Gather internal and external research)<br></br>
                              <input type="checkbox"
                                     className="form-control"
                                     ref="design"
                                     defaultValue={this.state.design}
                                     onBlur={this.validationCheck}
                                     onChange={this.onChangeThree} />Design - (Design something small to try)<br></br>
                              <input type="checkbox"
                                     className="form-control"
                                     ref="pdsa"
                                     defaultValue={this.state.pdsa}
                                     onBlur={this.validationCheck}
                                     onChange={this.onChangeFour} />PDSA - (Energize in PDSA cycles)<br></br>
                              <input type="checkbox"
                                     className="form-control"
                                     ref="synthesize"
                                     defaultValue={this.state.synthesize}
                                     onBlur={this.validationCheck}
                                     onChange={this.onChangeFive} />Synthesize - (Document and synthesize team learning)<br></br>
                              <input type="checkbox"
                                     className="form-control"
                                     ref="scale"
                                     defaultValue={this.state.scale}
                                     onBlur={this.validationCheck}
                                     onChange={this.onChangeSix} />Scale - (Share innovation with wider group)<br></br>
                    </div>
                </div>
              </div>
            </div>
